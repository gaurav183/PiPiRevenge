...
class PiPiRevenge(EventBasedAnimationClass):
    ...
    def initAnimation(self):
        ...
        self.startHeight = self.height/4
    ...
    def onTimerFired(self):
        self.startHeight += 15
        if (self.startHeight >= (7*self.height/8)):
          self.startHeight = self.height/4

    def drawAsteroids(self):
        self.asteroid = PhotoImage(file = "images/asteroid.gif")
        self.canvas.create_image((self.width/4), self.startHeight, image = self.asteroid)
        self.canvas.create_image((self.width/2), self.startHeight, image = self.asteroid)
        self.canvas.create_image((3*self.width/4), self.startHeight, image = self.asteroid)
